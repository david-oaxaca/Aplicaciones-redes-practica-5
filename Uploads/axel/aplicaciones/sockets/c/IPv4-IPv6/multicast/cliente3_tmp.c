...
struct dato{
 ...
 void diep(char *s)
  ...
  int main(int argc, char* argv[])
  {
    ...
    struct dato *o2;
    socklen_t ctam;
    int s,cd, i,v=1, op=0,n;
    unsigned char ttl = 50; 
   ...
   /* Resolve the multicast group address */
    ...
    hints.ai_protocol = 0;
    if ( getaddrinfo(GPO4, PORT, &hints, &result) != 0 )
    {
   ...
   if (rp == NULL) {               /* No address succeeded */
    ...
    if ( maddr->ai_family  == PF_INET && maddr->ai_addrlen == sizeof(struct sockaddr_in) ) /* IPv4 */
    {
        ...
        multicastRequest.imr_interface.s_addr = inet_addr("148.204.57.24");
	...
	if (setsockopt(cd, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &ttl,sizeof(ttl)) == -1) {
            perror("setsockopt TTL");
            exit(1);
        }//if
    ...
    for(;;){
	  ...
	  printf("antes de recvfrom\n");       
          if ((n=recvfrom(cd, buf, sizeof(buf), 0,(struct sockaddr *)&emisor, &ctam))==-1)
               diep("recvfrom()");
	     printf(" %d bytes recibidos1 con elmensaje %s\n",n,buf);	  
	     /*printf(" %d bytes recibidos1\n",n);
              o2 = (struct dato *)buf;
              printf(" %d bytes recibidos2\n",n);
          i = getnameinfo((struct sockaddr *) &emisor,ctam, host, NI_MAXHOST,service, NI_MAXSERV, NI_NUMERICSERV);
          if (i == 0){
