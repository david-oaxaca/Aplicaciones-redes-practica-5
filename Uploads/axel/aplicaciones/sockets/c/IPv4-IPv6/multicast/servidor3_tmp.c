...
/* Join the multicast group. We do this seperately depending on whether we
	...
	if (setsockopt(sd, IPPROTO_IPV6, IPV6_MULTICAST_HOPS, &ttl,sizeof(ttl)) == -1) {
            perror("setsockopt TTL");
            exit(1);
        }//if
...
*/
   ...
   if ( getaddrinfo(GPO4, "9931", &dst, &result) != 0 )
    {
        perror("getaddrinfo3() failed");
    }//if
    for(;;){
       printf("Enviando datos..\n");
       /*if (sendto(sd, (const char*)o1, sizeof(struct dato), 0, (struct sockaddr *)rp->ai_addr, rp->ai_addrlen)==-1)
          diep("sendto()"); */
