...
//int writeFd = open(pipeName, O_RDWR|O_NONBLOCK);
int rdwrFd = open(pipeName, O_RDWR|O_NONBLOCK);
// Now implement a reader and writer for these fds
// Launch Reader Thread

pthread_create( &readerThread, NULL, &reader, (void*) (&rdwrFd));
pthread_create( &writerThread, NULL, &writer, (void*) (&rdwrFd));
...
// Dump read data
printf("\nReader: %d\n", data);
//close(readFd); //cierro pipe deja de leer
//unlink(pipeName); //termino pipe deja de existir
} else {
