...
{  
    ...
    rc = setsockopt(slisten, SOL_SOCKET, SO_REUSEADDR,&val, sizeof 
(val) );
    if(rc<0){
       printf("No se pudo modificar opcion de socket\n");
       close(slisten);
       exit(0);
     }   
    
    rc = ioctl(slisten,FIONBIO, &val);
 if(rc<0){
       printf("No se pudo modificar opcion de socket\n");
       close(slisten);
       exit(0);
